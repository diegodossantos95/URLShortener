...
class NetworkClient {
    ...
    ///   - encodableObj: Encodable object representation of the request's body.
    ///   - completion: The completion handler to call when the load request is complete.
    public func postHTTP<T : Encodable, Y: Decodable>(at url: String, withEncodableObj encodableObj: T, withDecodableObj decodableObj: Y.Type, withCompletion completion: @escaping (Y?, HTTPRequestError?) -> Void) {
        guard let url = URL(string: url) else {
            completion(nil, .CouldNotFormURL)
            return
        }
        ...
        do {
            ...
            request(urlRequest) { (data, _, error) in
                if let data = data {
                    do {
                        let jsonDecoder = JSONDecoder()
                        let obj = try jsonDecoder.decode(decodableObj, from: data)
